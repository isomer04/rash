#!/usr/bin/env python3
"""
Deploy the Rash Financial Advisor Part 7 infrastructure.
This script:
...
def deploy_terraform():
    ...
    # The stack uses a partial backend configuration, so init requires the
    # gitignored backend.hcl. Run it every time, not just when .terraform is
    # missing: an existing .terraform records whichever backend was configured
    # last, which may no longer be what backend.hcl says. Terraform compares the
    # two and refuses to continue on a mismatch rather than planning against the
    # wrong state.
    ...
    if not backend_config.exists():
        ...
        print("     See terraform/README.md for the state setup.")
        sys.exit(1)
...
def main():
    ...
    print("🚀 Rash Financial Advisor - Part 7 Deployment")
    print("=" * 50)

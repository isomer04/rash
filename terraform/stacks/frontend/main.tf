...
#
# Both stacks keep their state in the shared bucket created by stacks/bootstrap,
# so these read from S3 rather than a sibling directory. The bucket name is
# derived the same way bootstrap builds it, which keeps the account ID out of
# the repository.
...
locals {
  ...
  # Must match `bucket` in every stack's backend.hcl.
  state_bucket = "rash-tfstate-${data.aws_caller_identity.current.account_id}"

  dynamodb_table_arns = values(data.terraform_remote_state.database.outputs.dynamodb_table_arns)
...
# Preserve the existing inline-policy state address during the database cutover.
moved {
  from = aws_iam_role_policy.api_lambda_aurora
  to   = aws_iam_role_policy.api_lambda_dynamodb
}

# Policy for DynamoDB application data access
    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:lambda:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:function:rash-planner",
          "arn:aws:lambda:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:function:rash-tagger",
          "arn:aws:lambda:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:function:rash-reporter",
          "arn:aws:lambda:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:function:rash-charter",
          "arn:aws:lambda:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:function:rash-retirement"
        ]
      }
    ]
  })
...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      # DynamoDB configuration from Part 5
      DYNAMODB_USERS_TABLE       = data.terraform_remote_state.database.outputs.dynamodb_table_names.users
      DYNAMODB_INSTRUMENTS_TABLE = data.terraform_remote_state.database.outputs.dynamodb_table_names.instruments
      DYNAMODB_ACCOUNTS_TABLE    = data.terraform_remote_state.database.outputs.dynamodb_table_names.accounts
      DYNAMODB_POSITIONS_TABLE   = data.terraform_remote_state.database.outputs.dynamodb_table_names.positions
      DYNAMODB_JOBS_TABLE        = data.terraform_remote_state.database.outputs.dynamodb_table_names.jobs
...
resource "aws_cloudfront_distribution" "main" {
  ...
  tags                = local.common_tags
  comment             = "Rash Financial Advisor Frontend"

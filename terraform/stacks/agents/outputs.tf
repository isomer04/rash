...
output "lambda_functions" {
  ...
  value = {
    planner    = module.agents["planner"].function_name
    tagger     = module.agents["tagger"].function_name
    reporter   = module.agents["reporter"].function_name
    charter    = module.agents["charter"].function_name
    retirement = module.agents["retirement"].function_name
  }
}
...
output "setup_instructions" {
  ...
  value       = <<-EOT
    ...
    Lambda Functions:
    - Planner (Orchestrator): ${module.agents["planner"].function_name}
    - Tagger: ${module.agents["tagger"].function_name}
    - Reporter: ${module.agents["reporter"].function_name}
    - Charter: ${module.agents["charter"].function_name}
    - Retirement: ${module.agents["retirement"].function_name}
    ...
    3. Monitor progress in CloudWatch Logs:
       - /aws/lambda/rash-planner
       - /aws/lambda/rash-tagger
       - /aws/lambda/rash-reporter
       - /aws/lambda/rash-charter
       - /aws/lambda/rash-retirement

"""
FastAPI backend for Rash Financial Advisor
...
app = FastAPI(
    title="Rash Financial Advisor API",
...
async def delete_account(account_id: str, clerk_user_id: str = Depends(get_current_user_id)):
    ...
    try:
        ...
        # Delete positions first; DynamoDB has no cascade, so ordering is ours to own
...
async def reset_accounts(clerk_user_id: str = Depends(get_current_user_id)):
    ...
    try:
        ...
        # Accounts.delete also removes the account's positions
        deleted_count = 0

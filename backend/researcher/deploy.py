...
def terraform_init(terraform_dir):
    """Initialise the stack's partial S3 backend before any other command."""
    backend_config = terraform_dir / "backend.hcl"
    if not backend_config.exists():
        raise SystemExit(
            f"Missing {backend_config}. Copy backend.hcl.example to backend.hcl "
            f"and fill it in; see terraform/README.md."
        )
    run_command(
        ["terraform", "init", "-backend-config=backend.hcl"], cwd=terraform_dir
    )
...
def main():
    print("Rash Researcher Service - Lambda Deployment")
    print("==========================================")
    ...
    terraform_dir = repo_root / "terraform" / "stacks" / "researcher"
    backend_dir = repo_root / "backend" / "researcher"

    print("\nInitialising Terraform backend...")
    terraform_init(terraform_dir)

    print("Ensuring Terraform ECR prerequisites exist...")
    terraform_apply(
        terraform_dir,
        targets=[
    ...
    local_image = f"rash-researcher:{image_tag}"

"""
Rash Researcher Service - Investment Advice Agent
"""
...
from mcp_servers import create_playwright_mcp_server, env_int
from tools import ingest_financial_document

# Load environment
load_dotenv(override=True)

app = FastAPI(title="Rash Researcher Service")


MCP_LOGGING_ENABLED = os.getenv("MCP_LOGGING") == "True"

# Every turn resends the whole transcript, and with Playwright in the loop that
# transcript carries scraped page text - so input tokens grow quadratically in
# the turn count and dominate the Bedrock bill. Observed runs settle around 5
# turns, so this ceiling only clips the pathological ones.
RESEARCH_MAX_TURNS = env_int("RESEARCH_MAX_TURNS", 6)
...
async def run_research_agent(topic: str = None) -> str:
    ...
    with trace("Researcher"):
        async with create_playwright_mcp_server(timeout_seconds=120) as playwright_mcp:
            ...
            agent = Agent(
                name="Rash Investment Researcher",
...
async def health():
    ...
    return {
        "service": "Rash Researcher",
        "status": "healthy",
        "rash_api_configured": bool(os.getenv("RASH_API_ENDPOINT") and os.getenv("RASH_API_KEY")),
        "timestamp": datetime.now(UTC).isoformat(),

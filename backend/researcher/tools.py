"""
Tools for the Rash Researcher agent
"""
import os
...
RASH_API_ENDPOINT = os.getenv("RASH_API_ENDPOINT")
RASH_API_KEY = os.getenv("RASH_API_KEY")
...
def _ingest(document: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with httpx.Client() as client:
        response = client.post(
            RASH_API_ENDPOINT,
            json=document,
            headers={"x-api-key": RASH_API_KEY},
            timeout=30.0
        )
...
def ingest_financial_document(topic: str, analysis: str) -> Dict[str, Any]:
    """
    Ingest a financial document into the Rash knowledge base.
    ...
    """
    if not RASH_API_ENDPOINT or not RASH_API_KEY:
        return {
            "success": False,
            "error": "Rash API not configured. Running in local mode."
        }

...
def taint_and_deploy_via_terraform() -> bool:
    ...
    terraform_dir = Path(__file__).parent.parent / "terraform" / "stacks" / "agents"
    if not terraform_dir.exists():
        print(f"❌ Terraform directory not found: {terraform_dir}")
        return False

    # The stack uses a partial backend configuration, so every terraform command
    # here needs an initialised backend from the gitignored backend.hcl.
    ...
    print("📌 Initialising Terraform backend...")
    init = subprocess.run(
    ...
    if init.returncode != 0:
        print(f"❌ terraform init failed: {init.stderr[:300]}")
        return False
    ...
    for func in lambda_functions:
        # The agents live in module.agents, keyed by agent name.
        address = f'module.agents["{func}"].aws_lambda_function.this'
...
def main():
    ...
    print("🎯 Deploying Rash Agent Lambda Functions (via Terraform)")
    print("=" * 50)
    ...
    if taint_and_deploy_via_terraform():
        ...
        print("   4. Check terraform state: cd terraform/stacks/agents && terraform plan")
        print("      (run terraform init -backend-config=backend.hcl there first)")
        sys.exit(1)

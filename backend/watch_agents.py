#!/usr/bin/env python3
"""
Watch CloudWatch logs from all Rash agents in real-time.
...
# Agent log groups
LOG_GROUPS = {
    'PLANNER': '/aws/lambda/rash-planner',
    'TAGGER': '/aws/lambda/rash-tagger',
    'REPORTER': '/aws/lambda/rash-reporter',
    'CHARTER': '/aws/lambda/rash-charter',
    'RETIREMENT': '/aws/lambda/rash-retirement',
}
...
class AgentLogWatcher:
    ...
    def watch(self, poll_interval: int = 2):
        ...
        print(f"{COLORS['BOLD']}Watching CloudWatch logs for all Rash agents...{COLORS['RESET']}")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Watch CloudWatch logs from all Rash agents')
    parser.add_argument(
        '--region',
        default='us-east-1',
